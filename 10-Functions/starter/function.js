...
// Call method
book.call(eurowings, 23, 'Salar');
console.log(eurowings);

// Apply method
const flightData = [134, 'Alee Van'];
...
const poll = {
  ...
  registerNewAnswer() {
    const promptQuestion = this.question + '\n' + this.options.join('\n');
    const option = Number(prompt(promptQuestion));
    console.log(typeof option, option);
    option < this.answers.length && this.answers[option]++;
    // console.log(this.answers.join(' '));
    this.displayResults();
    this.displayResults('string');
  },
  displayResults(type = 'array') {
    if (type === 'array') console.log(this.answers.join(' '));
    if (type === 'string') {
